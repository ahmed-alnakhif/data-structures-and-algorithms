...
|  9  | [Reorganize String](https://leetcode.com/problems/reorganize-string/) | [Java](src/Problems/topKElements/SumOfElements.java) | 1. HashMap for freq and Max Heap <br>if there's any char that has an occurance greater than (s.length() + 1) / 2 then there's no solution <br>3. poll max, if last element in our result is the same then poll again <br>4. add the element back if the freq is > 0  | T: O(N*log(N)) <br>S: O(N) |
